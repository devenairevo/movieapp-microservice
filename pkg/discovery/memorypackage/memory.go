...
func (r *Registry) ReportHealthyState(intstanceID string, serviceName string) error {
	r.Lock()
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
		return errors.New("service is not registered yet")
	}
	if _, ok := r.serviceAddrs[serviceName][intstanceID]; !ok {
		return errors.New("instance" + intstanceID + " of service " + serviceName + " is not registered yet")
	}

	r.serviceAddrs[serviceName][intstanceID].lastActive = time.Now()
	return nil
}
